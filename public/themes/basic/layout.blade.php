...
<div x-data="{ mobileMenuOpen: false }" class="relative min-h-screen flex flex-col pt-[61px]">
    ...
    <div x-show="mobileMenuOpen"
            ...
            <div class="mt-4 pt-4 border-t border-gray-100 shrink-0">
                @if(isset($_SESSION['user_id']))
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div id="mp-mobile-wrapper">
                            <button type="button" class="mp-mobile-btn" onclick="MusicController.toggle()">
                                <span id="mp-mobile-play-icon" class="mp-btn-icon">▶</span>
                            </button>
                        </div>
                        @endif
                    </div>
        ...
        <div class="p-4 bg-gray-50 text-center text-xs text-gray-400 shrink-0">
            &copy; {{ $group->name ?? '커스터드' }}
        </div>
    </div>

    @if(!empty($group->playlist))
    <div id="mp-pc-wrapper">
        <div class="mp-drag-handle" id="mp-drag-target">
            <div class="mp-drag-icon"></div>
        </div>
        
        <div class="mp-pc-content">
            <div class="flex justify-center items-center gap-4 mt-1">
                <button type="button" class="text-gray-500 hover:text-gray-800" onclick="MusicController.prev()">⏮</button>
                <button type="button" class="w-10 h-10 rounded-full bg-amber-600 text-white flex items-center justify-center hover:bg-amber-700 shadow-sm" onclick="MusicController.toggle()">
                    <span id="mp-pc-play-icon" class="mp-btn-icon">▶</span>
                </button>
                <button type="button" class="text-gray-500 hover:text-gray-800" onclick="MusicController.next()">⏭</button>
            </div>
            
            <input type="range" min="0" max="100" value="50" oninput="MusicController.setVolume(this.value)" class="w-full h-1 bg-gray-200 rounded-lg appearance-none cursor-pointer">
        </div>
    </div>
    @endif

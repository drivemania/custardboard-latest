...
<div class="max-w-5xl mx-auto py-8 relative" x-data="{ writeModal: false, editModal: false, editPreview: '', editAction: '' }">
    ...
    <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
        <div>
            ...
            <button type="button" onclick="window.open('{{ $base_path }}/emoticon', 'emoticon_pop', 'width=400,height=500,scrollbars=yes,resizable=no');"
             ...
             <button type="button" onclick="swup.navigate(window.location.href)" class="border border-neutral-200 hover:border-neutral-400 px-4 py-2 rounded-md transition-colors text-sm font-bold">새로고침</button>
                ...
                <div class="flex items-center gap-2 text-sm text-neutral-400">
                    ...
                    <button type="button" 
                            @click="editModal = true; 
                                    editPreview = '{{ $base_path }}{{ $doc->content }}'; 
                                    editAction = '{{ $currentUrl }}/{{ $doc->doc_num }}/edit'"
                            class="hover:text-amber-600">수정</button>
                    <span class="text-neutral-300">|</span>
                                    ...
                                    <div class="bg-white px-4 py-2 rounded-2xl rounded-tl-none border border-neutral-200 shadow-sm inline-block max-w-full">
                                        <div class="flex justify-between items-baseline gap-4 mb-1">
                                            <span id="comment_{{ $cmt->id }}" class="scroll-mt-24 target:bg-yellow-50 font-bold text-sm text-neutral-800">
                                                ...
                                                <a href="{{ $base_path }}/{{ $cmt->char_menu_slug }}/{{ $cmt->char_id }}" target="_blank">
                                                    @if(!empty($cmt->title_icon))
                                                    <img src="{{ $base_path }}{{ $cmt->title_icon }}" class="inline">
                                                    @endif
                                                    {{ $cmt->char_name }}
                                                </a>
                                                @endif
    ...
    <div x-show="writeModal"
        ...
        <div class="bg-white w-full h-full md:h-auto md:max-h-[85vh] md:max-w-lg md:rounded-2xl shadow-2xl flex flex-col overflow-hidden relative">
            ...
            <div class="p-4 border-t border-neutral-100 bg-neutral-50 shrink-0">
                <button type="button" onclick="document.getElementById('loadForm').requestSubmit();" class="w-full bg-amber-600 text-white font-bold py-3 rounded-xl shadow hover:bg-amber-700 active:scale-[0.98] transition-all">
                    업로드
                </button>
            </div>
    ...
    <div x-show="editModal"
        ...
        <div class="bg-white w-full h-full md:h-auto md:max-h-[85vh] md:max-w-lg md:rounded-2xl shadow-2xl flex flex-col overflow-hidden relative">
            ...
            <div class="px-5 py-4 border-b border-neutral-100 flex justify-between items-center bg-white shrink-0">
                <h3 class="text-lg font-bold text-neutral-800">로그 수정</h3>
                <button @click="editModal = false" class="text-neutral-400 hover:text-neutral-600 p-1 rounded-full hover:bg-neutral-100 transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                </button>
            </div>

            <div class="flex-1 overflow-y-auto p-5 scrollbar-hide">
                <form id="editForm" :action="editAction" method="POST" enctype="multipart/form-data" class="space-y-5">
                    
                    <div x-data="{ newPreview: null }">
                        <label class="block text-m font-bold text-neutral-700 mb-2">이미지 변경</label>
                        <div class="relative w-full h-64 border-2 border-dashed border-neutral-300 rounded-xl bg-neutral-50 flex flex-col items-center justify-center text-neutral-400 overflow-hidden hover:bg-neutral-100 hover:border-amber-300 transition-colors">
                            
                            <img :src="newPreview ? newPreview : editPreview" class="absolute inset-0 w-full h-full object-contain bg-neutral-100 z-10">
                            
                            <input type="file" name="content" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-20"
                                   @change="newPreview = URL.createObjectURL($event.target.files[0])">
                                   
                            <button type="button" x-show="newPreview" @click.prevent="newPreview = null; $el.parentNode.querySelector('input').value = ''" 
                                    class="absolute top-2 right-2 z-30 bg-black/60 text-white rounded-full p-1 hover:bg-black">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                            </button>
                        </div>
            ...
            <div class="p-4 border-t border-neutral-100 bg-neutral-50 shrink-0">
                <button type="button" onclick="document.getElementById('editForm').requestSubmit();" class="w-full bg-amber-600 text-white font-bold py-3 rounded-xl shadow hover:bg-amber-700 active:scale-[0.98] transition-all">
                    수정 완료
                </button>
            </div>

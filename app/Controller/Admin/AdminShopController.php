...
class AdminShopController {
    ...
    public function titleList(Request $request, Response $response) {
        $items = DB::table('titles')->where('is_deleted', 0)->orderBy('id', 'desc')->get();
        $content = $this->blade->render('admin.titles.index', [
            'titles' => $items
        ]);
        $response->getBody()->write($content);
        return $response;
    ...
    public function titleStore(Request $request, Response $response) {
        $data = $request->getParsedBody();
        $files = $request->getUploadedFiles();

        $iconPath = $data['existing_icon_path'] ?? null;
        if (isset($files['icon']) && $files['icon']->getError() === UPLOAD_ERR_OK) {
            $uploadDir = __DIR__ . '/../../../public/data/uploads/titles';
            if (!is_dir($uploadDir)) mkdir($uploadDir, 0777, true);
            
            $filename = uniqid() . '_' . $files['icon']->getClientFilename();
            $files['icon']->moveTo($uploadDir . '/' . $filename);
            $iconPath = '/public/data/uploads/titles/' . $filename;
        }
        ...
        if (!empty($data['id'])) {
            DB::table('titles')->where('id', $data['id'])->update($saveData);
            $msg = '타이틀이 수정되었습니다.';
        } else {
            DB::table(table: 'titles')->insert($saveData);
            $msg = '타이틀이 생성되었습니다.';
        }
    ...
    public function titleDelete(Request $request, Response $response) {
        $data = $request->getParsedBody();
        DB::table('character_titles')->where('title_id', $data['id'])->update([
            'is_deleted' => 1,
            'deleted_at' => date('Y-m-d H:i:s')
        ]);

        DB::table('titles')->where('id', $data['id'])->update([
            'is_deleted' => 1,
            'deleted_at' => date('Y-m-d H:i:s')
        ]);
        $_SESSION['flash_message'] = '삭제처리되었습니다.';
    ...
    public function settlementManage(Request $request, Response $response, $args) {
        ...
        $titles = DB::table('titles')->where('is_deleted', 0)->get();
    ...
    public function settlementDist(Request $request, Response $response) {
        ...
        $gaveTitles = [];
        if (!empty($titleIds)) {
            $gaveTitles = DB::table('titles')->whereIn('id', $titleIds)->select('id', 'name')->get()->toArray();
            ...
            if (!empty($titleInsertData)) {
                foreach (array_chunk($titleInsertData, 1000) as $chunk) {
                    DB::table('character_titles')->insertOrIgnore($chunk);
                }
            }
        }
        ...
        DB::table('settlement_logs')->insert([
            ...
            'titles_json' => !empty($gaveTitles) ? json_encode($gaveTitles, JSON_UNESCAPED_UNICODE) : null,
            'reason' => $reason,

...
<body class="bg-gray-50 h-screen flex flex-col">
    ...
    <header class="bg-white border-b px-4 py-3 flex justify-between items-center shrink-0">
    ...
    <div class="flex-1 flex overflow-hidden">
        <nav class="w-40 bg-white border-r flex flex-col py-4 shrink-0">
            ...
            <div class="bg-white rounded border shadow-sm">
                <table class="w-full text-left text-sm">
                    <thead class="bg-gray-50 text-gray-500 border-b">
                        <tr>
                            <th class="px-4 py-2 w-24">{{ $type == 'sent' ? '받는사람' : '보낸사람' }}</th>
                            <th class="px-4 py-2">내용</th>
                            <th class="px-4 py-2 w-24 text-right">날짜</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y">
                        ...
                        <tr class="hover:bg-gray-50 cursor-pointer transition {{ ($type == 'recv' && !$msg->read_at) ? 'bg-blue-50' : '' }}"
                            ...
                            <td class="px-4 py-3">
                                <div class="truncate w-64 {{ ($type == 'recv' && !$msg->read_at) ? 'font-bold text-gray-900' : 'text-gray-600' }}">
                                    {{ $msg->content }}
                                </div>
                            </td>

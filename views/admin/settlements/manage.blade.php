...
<div x-data="settlementsManager()" 
     @item-selected.window="addItem($event.detail.id, $event.detail.name)"
     @title-selected.window="addTitle($event.detail.id, $event.detail.name)">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-6">
                <div class="bg-white rounded-lg shadow p-6 sticky top-6">
                    ...
                    <div class="mb-4">
                        <label class="block text-sm font-bold text-neutral-700 mb-1">타이틀 지급</label>
                        
                        <select id="title-search" placeholder="타이틀을 검색해서 추가하세요..." autocomplete="off">
                            <option value="">타이틀 검색...</option>
                        ...
                        <div class="mt-3 space-y-2">
                            <template x-for="title in selectedTitles" :key="title.id">
                                <div class="flex items-center justify-between bg-neutral-50 p-2 rounded border border-neutral-200">
                                    <div class="flex items-center">
                                        <span class="text-sm font-bold text-amber-700" x-text="title.name"></span>
                                    </div>

                                    <div class="flex items-center space-x-2">
                                        <button type="button" @click="removeTitle(title.id)" class="text-neutral-400 hover:text-red-500 p-1">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                                        </button>
                                    </div>
                                    <input type="hidden" name="titles[]" :value="title.id">
                                </div>
                            </template>

                            <div x-show="selectedTitles.length === 0" class="text-xs text-neutral-400 text-center py-2">
                                선택된 타이틀이 없습니다.
                            </div>
                        </div>
                    </div>
...
function settlementsManager() {
    return {
        ...
        addItem(id, name) {
            ...
            this.selectedItems.push({ id: id, name: name, quantity: 1 });
        },
        ...
        addTitle(id, name) {
            const existing = this.selectedTitles.find(t => t.id == id);
            if (existing) {
                alert('이미 선택된 타이틀입니다. (타이틀은 중복 지급이 불가능합니다.)');
                return;
            }
            this.selectedTitles.push({ id: id, name: name });
        },

        removeTitle(id) {
            this.selectedTitles = this.selectedTitles.filter(t => t.id !== id);
        },
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    new TomSelect("#title-search", {
        ...
        onItemAdd: function(value, item) {
            const name = item.dataset.name || item.innerText;
            window.dispatchEvent(new CustomEvent('title-selected', { 
                detail: { id: value, name: name }
            }));
            this.clear();
        }
